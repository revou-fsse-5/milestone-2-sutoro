...
const Login: React.FC = () => {
  ...
  const handleSubmit = async (values: User) => {
    try {
      // Simulate a login by fetching data from the Platzi Fake Store API
      const response = await axios.get('https://api.escuelajs.co/api/v1/users');
      
      // Simulating a user login by finding the user with matching email
      const user = response.data.find((user: { email: string }) => user.email === values.email);
